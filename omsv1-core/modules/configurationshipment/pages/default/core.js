...
import { getShipmentConfiguration, saveStoreConfig } from '@modules/configurationshipment/services/graphql';
...
const ContentWrapper = (props) => {
    ...
    const [saveShipmentConfiguration] = saveStoreConfig();
    ...
    const dataShipment = [...data.getShipmentConfiguration];

    const tempData = {};
    const validationSchema = {};

    data?.getShipmentConfiguration?.map((parent) => {
                ...
                firstChild?.fields?.map((secondChild) => {
                    ...
                    if (secondChild.type === 'email') {
                        validationSchema[key] = Yup.object().shape({
                            value: Yup.string().nullable().matches(regexMultipleEmail, t('shipmentconfiguration:Invalid_email_format')),
                        });
                    }
                });
            } else {
...
const Core = (props) => {
    ...
    const pageConfig = {
        title: t('shipmentconfiguration:Shipment_Configuration'),
    };
    ...
    const { loading, data, refetch } = getShipmentConfiguration();
    ...
    if (!data) {
        window.toastMessage({
            open: true,
            text: t('shipmentconfiguration:Data_not_found'),
            variant: 'error',
        });
        setTimeout(() => {
            router.push('/');
        ...
        return (
            <Layout pageConfig={pageConfig}>
                ...
                >
                    {t('shipmentconfiguration:Data_not_found')}
                </div>
            </Layout>
        );
    }
