...
const Core = (props) => {
    ...
    const pageConfig = {
        title: t('prioritylocationbyzone:Upload_New_Priority_Location'),
    };

    const [uploadPriorityZone] = gqlService.uploadPriorityZone();
    const [downloadList, downloadListRes] = gqlService.downloadSampleCsv({ type: 'priority_loc_by_zone' });
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            binary: Yup.string().required(t('prioritylocationbyzone:This_is_a_Required_field')),
        }),
        onSubmit: (values) => {
            handleSubmit(values);
        },
