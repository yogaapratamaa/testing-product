...
import useStyles from '@modules/locationzone/pages/import/components/style';
    ...
    return (
        <>
            <Button
                ...
                onClick={() => router.push('/oms/locationzone')}
                variant="contained"
            ...
            <h2 className={classes.titleTop}>{t('master:uploadNewZone')}</h2>
            <Paper className={classes.container}>
                <span className={clsx(classes.textAttach, classes.label)}>{t('master:attachFile')}</span>
                <div className={classes.content}>
                    <div className={classes.formField}>
                        <span className={classes.label}>
                            <a href={urlDownload} className={classes.linkDownload}>
                                {t('master:downloadSampleCsv')}
                            </a>
                        </span>
                    </div>
                    <div className={clsx(classes.formField, classes.textLeft)}>
                        <DropFile
                            title={`${t('master:pleaseSelectTheFile')} : `}
                ...
                {activityState && ((activityState.run_status === 'running' || activityState.run_status === 'pending ') || showProgress)
                    ? (
                        <div className={classes.progressContainer}>
                            <Progressbar total={activityState?.data_total} value={activityState?.data_processed} title={t('master:progress')} />
                        </div>
                    )
                    : null}
                {firstLoad
                                        ...
                                        <TableBody>
                                            ...
                                            <TableRow>
                                                ...
                                                <TableCell className={classes.rightColumn}>
                                                    ...
                                                    >
                                                        {activityState.attachment ? t('master:download') : '-'}
                                                    </a>
                                                </TableCell>
                                            </TableRow>
                                        </TableBody>
