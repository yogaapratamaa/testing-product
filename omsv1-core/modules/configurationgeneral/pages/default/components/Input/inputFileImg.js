...
const InputFileImg = (props) => {
    ...
    const onDropAccepted = async (files) => {
        ...
        formik.setFieldValue(`${name}.filename`, filebase64[0].file.name);
        setPreview(filebase64[0].baseCode);
    };
        ...
        <div className={classes.formField} style={{ margin: '16px 0 12px 0' }}>
            ...
            <div className={classes.divField}>
                <div {...getRootProps({ className: 'dropzone' })}>
                    ...
                    {acceptedFiles.length === 0 && <span className={classes.textNoFile}>{t('generalconfiguration:No_file_chosen')}</span>}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', marginTop: '18px' }}>
                    {
                        (formik.values[name].value && acceptedFiles.length > 0) && (
                            <span className={clsx(classes.label, classes.comment)} style={{ marginTop: '0' }} key={acceptedFiles[0].path}>
                                {t('generalconfiguration:New_file')} {acceptedFiles[0].path}
                                &nbsp; | &nbsp;
                                <span 
                                ...
                                >
                                    {t('generalconfiguration:Delete')}
                                </span>
                            </span>
                        )
                    }
                    {preview && <img src={preview} alt="preview_img" style={{ maxWidth: 150 }} />}
                    {
                        (formik.values[name].value && acceptedFiles.length === 0 && value) && (
                            <span className={clsx(classes.label, classes.comment)} style={{ marginTop: '0' }}>
                                {t('generalconfiguration:Old_file')} {value}
                                &nbsp; | &nbsp;
                                <span 
                                    className='link-button'
                                    style={{ cursor: 'pointer' }}
