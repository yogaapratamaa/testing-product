...
import { getRmaConfiguration, saveStoreConfig } from '@modules/configurationrma/services/graphql';
...
const ContentWrapper = (props) => {
    ...
    const [saveRmaConfiguration] = saveStoreConfig();

    const handleSubmit = (input) => {
        const tempArray = [];
        for (const key in input) {
            if (input[key].type === 'file') {
                // if there is a new (base64) file upload will be saved
    ...
    const dataReturn = [...data.getRmaConfiguration];

    const tempData = {};
    const validationSchema = {};

    data?.getRmaConfiguration?.map((parent) => {
                ...
                firstChild?.fields?.map((secondChild) => {
                    ...
                    if (secondChild.type === 'form') {
                        ...
                        const validation = Object.keys(value?.[0]).map((kv) => (
                            [kv, Yup.string().required(t('returnconfiguration:This_is_a_Required_field'))]
                        ));
                        validationSchema[key] = Yup.object().shape({
                            value: Yup.array().of(Yup.object().shape(Object.fromEntries(validation)))
                                .required(t('returnconfiguration:This_is_a_Required_field')),
                        });
                    }
                    ...
                    if (secondChild.type === 'email') {
                        validationSchema[key] = Yup.object().shape({
                            value: Yup.string().nullable().matches(regexMultipleEmail, t('returnconfiguration:Invalid_email_format')),
                        });
                    }
                });
            } else {
...
const Core = (props) => {
    ...
    const pageConfig = {
        title: t('returnconfiguration:Return_Configuration'),
    };

    const { loading: aclCheckLoading, data: aclCheckData } = aclService.isAccessAllowed({
        acl_code: 'configuration_rma',
    });

    const { loading, data, refetch } = getRmaConfiguration();
    ...
    if (!data) {
        window.toastMessage({
            open: true,
            text: t('returnconfiguration:Data_not_found'),
            variant: 'error',
        });
        setTimeout(() => {
            router.push('/');
        ...
        return (
            <Layout pageConfig={pageConfig}>
                ...
                >
                    {t('returnconfiguration:Data_not_found')}
                </div>
            </Layout>
        );
    }
