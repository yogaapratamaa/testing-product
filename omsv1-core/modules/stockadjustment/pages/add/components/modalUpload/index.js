...
const ModalFindProduct = (props) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            binary: Yup.string().required(t('stockadjustment:required')),
        }),
        onSubmit: async (values) => {
            ...
            try {
                if (!locationId) {
                    throw new Error(t('stockadjustment:locationRequired'));
                }
                const res = await csvToArrayOfObject({ variables });
                const data = res?.data?.csvToArrayOfObject ?? null;
                if (!data) {
                    throw new Error(t('stockadjustment:uploadFailed'));
                }
                ...
                if (SKU_IDX === -1 || STOCK_IDX === -1) {
                    throw new Error(t('stockadjustment:csvMissingHeader'));
                }
    ...
    useEffect(() => {
        ...
        if (sourceData && sourceData.getSourceList && sourceData.getSourceList.items && !sourceLoading) {
            ...
            if (data?.length === 0) {
                window.toastMessage({
                    open: true,
                    text: t('stockadjustment:noProductFound'),
                    variant: 'error',
                });
                return;
            }
            ...
            if (data?.length !== mappedSku?.length) {
                window.toastMessage({
                    open: true,
                    text: t('stockadjustment:someProductNotFound'),
                    variant: 'error',
                });
                return;
            }
