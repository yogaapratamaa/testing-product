...
class NodeModel {
    ...
    toggleChecked(node, isChecked, checkModel, noCascade, percolateUpward = true, allowParentOnlyChecked = false) {
        ...
        if (percolateUpward && !noCascade && flatNode.isChild && modelHasParents) {
            this.toggleParentStatus(flatNode.parent, checkModel, allowParentOnlyChecked);
        }

        return this;
    }

    toggleParentStatus(node, checkModel, allowParentOnlyChecked = false) {
        const flatNode = this.flatNodes[node.value];
        if (flatNode.isChild) {
            if (checkModel === CheckModel.ALL) {
                this.toggleNode(node.value, 'checked', allowParentOnlyChecked ? !node.checked : this.isEveryChildChecked(flatNode));
            }
