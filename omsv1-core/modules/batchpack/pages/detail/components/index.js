...
const BatchListPickListContent = (props) => {
    ...
    return (
        <>
            <Button className={classes.btnBack} onClick={() => router.push('/pickpack/batchpack')} variant="contained" style={{ marginRight: 10 }}>
                <ChevronLeftIcon
                    style={{
                        fontSize: 30,
                        position: 'absolute',
                ...
                <div className={classes.headerContent}>
                    <div style={{ marginBottom: 10 }}>
                        <span
                            ...
                            className={getValueStatus(packList.statusLabel || packList.statusValue)}
                        >
                            {packList.statusLabel || packList.statusValue}
                        </span>
                        <span>{`${t('packlist:Batch')} #${packList?.pick_increment_id || '-'}`}</span>
                    </div>
                    <div>
                        <span style={{ fontSize: 18 }}>{`${t('packlist:Slot')}: ${packList.slot || '-'}`}</span>
                    </div>
                    <div style={{ marginBottom: 10 }}>
                    ...
                    <div className={classes.content} key={list.shipment_id}>
                        <div className={classes.gridList}>
                            ...
                            <div style={{ textAlign: 'center' }}>
                                ...
                                <h5 className={clsx(classes.bodyList, 'bold', list.qty_picked !== list.qty && 'red')} style={{ fontSize: 18 }}>
